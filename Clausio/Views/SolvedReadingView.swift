...
// MARK: - Post-Solve Story Reading Screen
struct SolvedReadingView: View {
  ...
  /// Reconstructs each sentence from solved tile positions,
  /// grouped by originalRowId and sorted by originalColumnId.
  private var sentences: [(rowId: Int, fullText: String, chunks: [(text: String, furigana: String)])] {
  ...
  private func readAllSentences() {
    ...
    // Pause briefly between sentences so the listener can parse each one
...
struct SentenceCardView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 10) {
      ...
      // Scrollable furigana + chunk chips
      ScrollView(.horizontal, showsIndicators: false) {
        HStack(alignment: .top, spacing: 6) {
          ForEach(Array(chunks.enumerated()), id: \.offset) { _, chunk in
            ...
            .padding(.vertical, 3)
            .background(RoundedRectangle(cornerRadius: 5).fill(accentColor.opacity(0.08)))
          }
        }
        .padding(.vertical, 2)
      }
